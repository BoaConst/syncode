    ...
    let matches = Command::new("dvcs")
        ...
        .subcommand(
            Command::new("init")
                ...
                .arg(Arg::new("path").help("The path to your repository").required(false))
    ...
    match matches.subcommand() {
        Some(("init", init_matches)) => {
            ...
            if init_matches.args_present() {
                // let name = init_matches.get_one::<String>("path").unwrap().to_string();
                // let cwd = machine_hiding::file_system_operations::get_cwd();
                // directory = cwd + "/" + &name;
                directory = init_matches.get_one::<String>("path").unwrap().to_string();
            } else {
        ...
        // Some(("merge", merge_matches)) => {
        //     let mut args = Vec::new();
        //     let commit1 = merge_matches.get_one::<String>("commit1").unwrap().to_string();
        //     let commit2 = merge_matches.get_one::<String>("commit2").unwrap().to_string();
        //     args.push(&commit1);
        //     args.push(&commit2);
        //     user_hiding::user_interface::execute_command("merge".to_string(), args);
        // }
