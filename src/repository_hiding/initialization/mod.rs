...
impl Rev {
    ...
    pub fn checkout(&self) {
        for f_rel_path in &self.rev.files {
            println!("{}", f_rel_path);
            assert!(Path::new(f_rel_path).exists(), "File missing in a revision!");
...
pub fn new_revID() -> RevID {
    RevID {
        value: Uuid::new_v4().simple().into_uuid()
    }
}
...
pub fn rev_open(repo: &Repo, rev_id: &RevID) -> Rev {
    let rev_path = machine_hiding::file_system_operations::join_paths(&repo.dev_path, &rev_id.value.to_string());
    println!("{}", rev_path);
